	...
	float specPower)
{
	...
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, texture, lightDirection, ambientColor, diffuseColor, cameraPos, specularColor, specPower);
	if(!result)
	{
		return false;
	}
...
bool LightShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	result = device->CreateBuffer(&cameraBufferDesc, NULL, &m_cameraSet);
	if (FAILED(result))
	{
		return false;
	}
...
bool LightShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, 
										   D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, D3DXVECTOR3 lightDirection, D3DXVECTOR4 ambientColor,
										   D3DXVECTOR4 diffuseColor, D3DXVECTOR3 cameraPos, D3DXVECTOR4 specularColor, float specPow)
{
	HRESULT result;
	...
	result = deviceContext->Map(m_cameraSet, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	if(FAILED(result))
	{
		return false;
	}
	...
	deviceContext->Unmap(m_cameraSet, 0);

	bufferNumber = 1;

	deviceContext->VSSetConstantBuffers(bufferNumber, 1, &m_cameraSet);
